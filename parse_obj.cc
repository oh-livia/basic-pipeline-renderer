...
//
// If the file given is a bezier file, run the appropriate methods instead
//
void read_obj_file(const char *file, std::vector< int > &tris, std::vector< float > &verts)
{
    ...
    if (cmd.find_first_of("0123456789") != std::string::npos)
    {
        isBezier = true;
        in.close();
        read_wavefront_file(file);
    }
    
    else {
    
        for (int line=2; in.good(); line++) {
            in.getline(buffer,1024);
            buffer[in.gcount()]=0;
...
//Given the name of a bezier file, read in the uv curves and save as bezObjs
